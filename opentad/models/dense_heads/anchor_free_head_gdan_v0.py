...
from ..bricks import ConvModule, Scale, GradientReversalLayer, CrossNegativeAttention, TransformerBlock, MaskMambaBlock
...
class AnchorFreeHeadGDAN(nn.Module):
    def __init__(
        ...
        num_dom_heads=16,
        dom_ffn_type='ConvModule', #'Mamba', 'Transformer',
    ):
    ...
    def forward_train(self, feat_list, mask_list, gt_segments, gt_labels, gt_domains, **kwargs):
        ...
        for l, (feat, mask) in enumerate(zip(feat_list, mask_list)):
            # cls_feat = feat
            # reg_feat = feat
            dom_feat = self.grl_0(feat)
            dom_feat, _mask = self.dom_ffn(feat, mask)
            cls_feat, _mask = self.cna_cls(feat, self.grl_cls(dom_feat), mask)  
            reg_feat, _mask = self.cna_reg(feat, self.grl_reg(dom_feat), mask)
