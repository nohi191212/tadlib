...
class AnchorFreeHeadGDTAD(nn.Module):
    def __init__(
        self,
        num_classes,
        num_domains,
        ...
        super(AnchorFreeHeadGDTAD, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.cna = CrossNegativeAttention(n_embd=self.feat_channels, n_head=self.num_dom_heads)
        self.domain_generator = CrossAttention(n_embd=self.feat_channels, n_head=self.num_dom_heads)
    ...
    def forward_train(self, feat_list, mask_list, gt_segments, gt_labels, gt_domains, 
                      stage=1, t_cls_pred=None, t_reg_pred=None, gen_dom_shifts=None, **kwargs):
        if stage == 1:
            cls_pred = []
            reg_pred = []
            dom_pred = []
            ...
            for l, (feat, mask) in enumerate(zip(feat_list, mask_list)):
                ...
                nodom_feat, _mask = self.cna(feat, dom_feat, mask)
                cls_feat = nodom_feat
                reg_feat = feat
                ...
                dom_shift = self.stop_gradient((feat - nodom_feat).detach())
                ...
                recons_shift, _mask = self.domain_generator(feat.detach(), dom_feat.detach(), mask.detach())
                
                dom_shifts.append(dom_shift)
                recons_shifts.append(recons_shift)
            ...
            losses, pos_mask, gt_cls = self.losses_stage1(cls_pred, reg_pred, dom_pred, mask_list, points, 
                                            ...
                                            recons_shifts, dom_shifts, # gen_dom_loss
                                        )
            
            # for tSNE, dom_shifts是学习自然得到的偏移，recons_shifts是生成的偏移
            features, labels, domains, pos_dom_shifts, pos_recons_shifts = \
                self.get_pos_data(feat_list, pos_mask, gt_cls, gt_domains, dom_shifts, recons_shifts=recons_shifts)
            ...
            for l, (feat, mask, gen_dom_shift) in enumerate(zip(feat_list, mask_list, gen_dom_shifts)):
                dom_feat, _mask = self.dom_proj(feat, mask)
                nodom_feat, _mask = self.cna(feat, dom_feat, mask)
                cls_feat = nodom_feat
                reg_feat = feat
                ...
                # for make fig
                feat_noise = feat + torch.randn_like(feat) * 1.0
                dom_feat_noise, _mask = self.dom_proj(feat_noise, mask)
                nodom_feat_noise, _mask = self.cna(feat_noise, dom_feat_noise, mask)
                dom_shift_noise = feat_noise - nodom_feat_noise
                
                aug_feat = feat + gen_dom_shift
                aug_dom_feat, _mask = self.dom_proj(aug_feat, mask)
                aug_nodom_feat, _mask = self.cna(aug_feat, aug_dom_feat, mask)
                aug_cls_feat = aug_nodom_feat
                aug_reg_feat = aug_nodom_feat
                ...
                dom_shifts.append(dom_shift)
                nodom_feats.append(nodom_feat)
                aug_nodom_feats.append(aug_nodom_feat)

                # make fig
                dom_shifts_noise.append(dom_shift_noise)
            ...
            losses, pos_mask, gt_cls = self.losses_stage2(cls_pred, reg_pred, mask_list, points, 
                                            ...
                                            nodom_feats, aug_nodom_feats,
                                        )
            
            # for tSNE
            features, labels, domains, pos_dom_shifts, pos_gen_shifts, pos_dom_shifts_noise = \
                self.get_pos_data(feat_list, pos_mask, gt_cls, gt_domains, dom_shifts, gen_shifts=gen_dom_shifts,
                                  dom_shifts_noise=dom_shifts_noise)
            features = self.hook_feat(features).detach().cpu()
            labels = self.hook_label(labels).detach().cpu()
            domains = self.hook_domain(domains).detach().cpu()
            pos_dom_shifts = self.hook_dom_shift(pos_dom_shifts).detach().cpu()
            ...
            # make fig
            pos_dom_shifts_noise = self.hook_dom_shift_noise(pos_dom_shifts_noise).detach().cpu()
    ...
    def forward_distill(self, feat_list, mask_list, gt_segments, gt_labels, **kwargs):        
        ...
        for l, (feat, mask) in enumerate(zip(feat_list, mask_list)):
            ...
            dom_shift, _mask = self.domain_generator(feat, gen_domain_l, mask)
        ...
        return cls_pred, reg_pred, dom_shifts
    ...
    def get_pos_data(self, feat_list, pos_mask, gt_cls, gt_domains, 
        ...
        if gen_shifts is not None:
            ...
            if dom_shifts_noise is None:
                return pos_feats, pos_labels, pos_domains, pos_dom_shifts, pos_gen_shifts
            else:
                pos_dom_shifts_noise = torch.cat(dom_shifts_noise, dim=-1).transpose(1,2)[pos_mask]
                return pos_feats, pos_labels, pos_domains, pos_dom_shifts, pos_gen_shifts, pos_dom_shifts_noise
    ...
    def losses_stage1(
        ...
        recons_shifts = torch.cat(recons_shifts, dim=-1).permute(0, 2, 1)[valid_mask.detach()]
        dom_shifts = torch.cat(dom_shifts, dim=-1).permute(0, 2, 1)[valid_mask.detach()]
        gen_dom_loss = self.gen_dom_loss(recons_shifts, dom_shifts)
        gen_dom_loss /= loss_normalizer
    ...
    def losses_stage2(
        ...
        nodom_feats = [x.permute(0, 2, 1) for x in nodom_feats]
        nodom_feats = torch.cat(nodom_feats, dim=1)
        aug_nodom_feats = [x.permute(0, 2, 1) for x in aug_nodom_feats] # B, sumT, C
        aug_nodom_feats = torch.cat(aug_nodom_feats, dim=1)
        neg_da_loss = self.feat_da_loss(nodom_feats[neg_mask], aug_nodom_feats[neg_mask]) * 0.1
        pos_da_loss = self.feat_da_loss(nodom_feats[pos_mask], aug_nodom_feats[pos_mask])
        ...
        losses = {
            ...
            "neg_da_loss": neg_da_loss,
            "pos_da_loss": pos_da_loss,
        }
...
@HEADS.register_module()
class ActionFormerHeadGDTAD(AnchorFreeHeadGDTAD):
    def __init__(
        self,
        num_classes,
        num_domains,
