...
class AnchorFreeHeadMixup(nn.Module):
    def __init__(
        self,
        num_classes,
        in_channels,
        ...
        super(AnchorFreeHeadMixup, self).__init__()
    ...
    def forward_train(self, feat_list, mask_list, gt_segments, gt_labels, mixup_info, gt_domains, **kwargs):
        # gt_labels here is one-hot encoded
        cls_pred = []
        reg_pred = []
        ...
        losses, pos_mask, gt_cls = self.losses(cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels, mixup_info)
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels, mixup_info):
        # gt_segment:
        # gt_labels:
        gt_cls, gt_reg = self.prepare_targets(points, gt_segments, gt_labels, mixup_info)
    ...
    @torch.no_grad()
    def prepare_targets(self, points, gt_segments, gt_labels, mixup_info):
        # mixup_info: list of tuples (original_idx, mixup_idx, origin_weight, mixup_weight, len_origin/len_mixup)
        concat_points = torch.cat(points, dim=0)
        num_pts = concat_points.shape[0]
        gt_cls, gt_reg = [], []

        for idx in range(len(mixup_info)):
            origin_idx, mixup_idx, origin_weight, mixup_weight, scale_rate = mixup_info[idx]
            gt_segment = gt_segments[origin_idx]
            gt_label = gt_labels[origin_idx]
            gt_segment_mixup = gt_segments[mixup_idx] * scale_rate
            gt_label_mixup = gt_labels[mixup_idx]
            
            # mixup
            assert origin_idx == idx, mixup_info
            if mixup_weight > 0:
                gt_weight = [origin_weight]*gt_segment.shape[0] + [mixup_weight]*gt_segment_mixup.shape[0]
                gt_weight = torch.tensor(gt_weight).to(gt_segment.device) # N1+N2
                gt_segment = torch.cat((gt_segment, gt_segment_mixup), dim=0) # N1+N2 x 2
                gt_label = torch.cat((gt_label, gt_label_mixup), dim=0) # N1+N2 x C
            ...
            gt_segs = gt_segment[None].expand(num_pts, num_gts, 2) # sumT x N x 2
            ...
            min_len_mask = min_len_mask.to(reg_targets.dtype) # FT x N

            # cls_targets: F T x C; reg_targets F T x 2
            gt_label_one_hot = F.one_hot(gt_label.long(), self.num_classes).to(reg_targets.dtype) # N1+N2 x C or N x C
            if mixup_weight > 0:
                gt_label_one_hot *= gt_weight[:, None] # N1+N2 x C
            
            cls_targets = min_len_mask @ gt_label_one_hot # F T x C
...
@HEADS.register_module()
class ActionFormerHeadMixup(AnchorFreeHeadMixup):
    def __init__(
        self,
        num_classes,
        in_channels,
