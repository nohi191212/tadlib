...
from ..bricks import ConvModule, Scale, GradientReversalLayer, TransformerBlock


@HEADS.register_module()
class AnchorFreeHeadMMDAAE(nn.Module):
    def __init__(
        self,
        num_classes,
        num_domains,
        ...
        super(AnchorFreeHeadMMDAAE, self).__init__()
        ...
        self.mmdloss_weight = mmdloss_weight
        ...
        # for Pseudo Domain Discriminator
        # self.dom_ffn = ConvModule(in_channels=self.feat_channels, out_channels=self.feat_channels,
        #                     kernel_size=3, stride=1, padding=1, 
        #                     norm_cfg=dict(type="LN"), act_cfg=dict(type="relu"),)
        ...
        self.recons_loss = build_loss(loss.recons_loss)
        self.mmd_loss = build_loss(loss.mmd_loss)
    ...
    def _init_layers(self):
        ...
        self.grl = GradientReversalLayer(lambda_=2.0)
    ...
    def _init_decoder(self):
        # [Transformer*2, ConvModule*1, ConvModule*1] only reconstruct the first feature map
        arch = [2, 1, 1]
        ...
        self.decoder.append(
            ConvModule(
                in_channels=self.feat_channels,
                out_channels=self.origin_channels,
                kernel_size=1,
                stride=1,
                padding=0,
                norm_cfg=None,
    ...
    def forward_train(self, feat_list, mask_list, gt_segments, gt_labels, gt_domains, init_feats, proj_feats, **kwargs):
        cls_pred = []
        reg_pred = []
        dom_pred = []
        
        # for ActivityNet v1.3
        if self.num_classes == 1:
            gt_labels = [label.zero_() for label in gt_labels]
    ...
    def losses(self, cls_pred, reg_pred, dom_pred, mask_list, points, gt_segments, gt_labels, 
               gt_domains, init_feats, recons_feats, avg_feats):
        ...
        gt_target += self.label_smoothing / self.num_classes
        ...
        recons_loss = self.recons_loss(init_feats, recons_feats)
        recons_loss /= loss_normalizer
        
        # 5. MMD loss
        # avg_feats (B, C)
        domain_0_mask = torch.cat(gt_domains) == 0
        domain_1_mask = torch.cat(gt_domains) == 1
        domain_2_mask = torch.cat(gt_domains) == 2
        domain_0_exists = domain_0_mask.sum() > 0
        domain_1_exists = domain_1_mask.sum() > 0
        domain_2_exists = domain_2_mask.sum() > 0
        if int(domain_0_exists) + int(domain_1_exists) + int(domain_2_exists) >= 2:
            if not domain_0_exists:
                mmd_loss = self.mmd_loss(avg_feats[domain_1_mask], avg_feats[domain_2_mask])
            elif not domain_1_exists:
                mmd_loss = self.mmd_loss(avg_feats[domain_0_mask], avg_feats[domain_2_mask])
            elif not domain_2_exists:
                mmd_loss = self.mmd_loss(avg_feats[domain_0_mask], avg_feats[domain_1_mask])
        else:
            mmd_loss = torch.tensor(0.0).to(dom_pred.device)
        mmd_loss /= loss_normalizer
        ...
        losses = {
            ...
            "dom_loss": dom_loss * 3 * self.mmdloss_weight ,
            "recons_loss": recons_loss * 10 * self.mmdloss_weight ,
            "mmd_loss": mmd_loss * 3 * self.mmdloss_weight,
        }
...
@HEADS.register_module()
class ActionFormerHeadMMDAAE(AnchorFreeHeadMMDAAE):
    def __init__(
        self,
        num_classes,
        num_domains,
