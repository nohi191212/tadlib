...
class DiffTransformerBlock(nn.Module):
    """
    reproduct from https://arxiv.org/abs/2410.05258
    ...
    def __init__(
        ...
        path_pdrop=0.0,  # drop path rate
        mha_win_size=-1,  # > 0 to use window mha
        lambda_init=0.25, # a constant used to initialize the lambda parameter in the diff attention
    ):
        super().__init__()
        assert len(n_ds_strides) == 2
        ...
        if mha_win_size > 1:
            ...
            raise NotImplementedError
            # TODO: add diff attention
            # self.attn = MaskedDiffMHCA(
            #     in_channels,
            #     n_head,
            #     n_qx_stride=n_ds_strides[0],
            #     n_kv_stride=n_ds_strides[1],
...
@MODELS.register_module()
class LocalMaskedDiffMHCA(LocalMaskedMHCA):
    """
        Not Implemented Yet
    """
    ...
    def __init__(
        ...
        proj_pdrop=0.0,  # dropout rate for projection op
        use_rel_pe=False,  # use relative position encoding
        lambda_init=0.25,  # initialization value for the learnable lambda
    ):
        super().__init__()
        assert n_embd % n_head == 0
        self.n_embd = n_embd
        self.n_head = n_head
        self.head_dim = n_embd // n_head // 2 
        ...
        # key, query, value projections for all heads
        # it is OK to ignore masking, as the mask will be attached on the attention
        self.key = nn.Conv1d(self.n_embd, self.head_dim * 2, 1)
        self.query = nn.Conv1d(self.n_embd, self.head_dim * 2, 1)
        self.value = nn.Conv1d(self.n_embd, self.head_dim * 2, 1)
    ...
    def forward(self, x, mask):
        ...
        # step 2: query, key, value transforms & reshape
        # projections
        q1, q2 = torch.split(self.query(q), self.n_embd, dim=1) # B, T, d_h(64)
        k1, k2 = torch.split(self.key(k), self.n_embd, dim=1) # B, T, d_h(64)
        v = self.value(v) # B, T, 2*d_h
        # (B, nh * hs, T) -> (B, nh, T, hs)
        q1 = q1.view(B, self.n_head, self.head_dim, -1).transpose(2, 3)
        k1 = k1.view(B, self.n_head, self.head_dim, -1).transpose(2, 3)
        q2 = q2.view(B, self.n_head, self.head_dim, -1).transpose(2, 3)
        k2 = k2.view(B, self.n_head, self.head_dim, -1).transpose(2, 3)
        v = v.view(B, self.n_head, 2*self.head_dim, -1).transpose(2, 3)
        # view as (B * nh, T, hs)
        q1 = q1.view(B * self.n_head, -1, self.head_dim).contiguous()
        k1 = k1.view(B * self.n_head, -1, self.head_dim).contiguous()
        q2 = q2.view(B * self.n_head, -1, self.head_dim).contiguous()
        k2 = k2.view(B * self.n_head, -1, self.head_dim).contiguous()
        v = v.view(B * self.n_head, -1, 2*self.head_dim).contiguous()

        # step 3: compute local self-attention with rel pe and masking
        q1 *= self.scale
        q2 *= self.scale
        # chunked query key attention -> B, T, nh, 2w+1 = window_size
        att1 = self._sliding_chunks_query_key_matmul(q1, k1, self.n_head, self.window_overlap)
        att2 = self._sliding_chunks_query_key_matmul(q2, k2, self.n_head, self.window_overlap)
        ...
        inverse_kv_mask = torch.logical_not(kv_mask[:, None, :, None].view(B, -1, 1))
        # 0 for valid slot, -inf for masked ones
        float_inverse_kv_mask = inverse_kv_mask.type_as(q1).masked_fill(inverse_kv_mask, -1e4)
        # compute the diagonal mask (for each local window)
        diagonal_mask = self._sliding_chunks_query_key_matmul(
            float_inverse_kv_mask.new_ones(size=float_inverse_kv_mask.size()),
            float_inverse_kv_mask,
        ...
        # ignore input masking for now
        att1 = nn.functional.softmax(att1, dim=-1)
        att2 = nn.functional.softmax(att2, dim=-1)
        
        # calculate true attention
        _lambda = torch.exp(self.lambda_q1 * self.lambda_k1) - torch.exp(self.lambda_q2 * self.lambda_k2) \
            + self.lambda_init
        att = att1 - _lambda * att2
