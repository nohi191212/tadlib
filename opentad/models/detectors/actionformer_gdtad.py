...
class ActionFormerGDTAD(SingleStageDetector):
    def __init__(
        self,
        projection,
        rpn_head,
    ...
    def forward_test(self, inputs, masks, metas=None, infer_cfg=None, mode='infer', **kwargs):
        if self.with_backbone:
            x = self.backbone(inputs)
        else:
            x = inputs
        ...
        if mode == 'infer':
            ...
            results = self.rpn_head.forward_distill(x, masks, **kwargs)
            return results
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith("bias"):
                    ...
                    no_decay.add(fpn)
                elif pn.endswith((".A_log",".D", ".learnable_var")):
                    # corner case for mamba block, LearnableRegLoss
                    no_decay.add(fpn)
